// @cardapio/services/complementos/vincular-complementos-combo.ts
...
/**
 * Parâmetros para vincular complementos a um combo
 */
export interface VincularComplementosComboParams {
  combo_id: number;
  request: VincularComplementosRequest | VincularComplementosRequestSimples;
}

/**
 * Hook para vincular complementos a um combo
 ...
 * const vincularComplementos = useVincularComplementosCombo();
 ...
 * ```
 */
export function useVincularComplementosCombo() {
  const qc = useQueryClient();
  
  const invalidateQueries = () => {
    qc.invalidateQueries({ queryKey: ["complementos-unificado"], exact: false });
    qc.invalidateQueries({ queryKey: ["combos"], exact: false });
    qc.invalidateQueries({ queryKey: ["complementos"], exact: false });
  };

  return useMutation({
    mutationFn: async (params: VincularComplementosComboParams) => {
      await apiAdmin.post(
        `/api/catalogo/admin/combos/${params.combo_id}/complementos`,
        params.request
      );
    },
    onSuccess: () => {
