...
import { formatDisplayDate, formatDateToYYYYMMDD } from "@supervisor/utils/format/formatDate";
...
import LoadingModal from "@supervisor/components/shared/modals/LoadingModal";


interface Props {
  empresasDetalhes: { empr_codigo: string; empr_nomereduzido: string }[];
  onSuccess: (data: TypeDashboardResponse) => void;
}

export default function ComponentCardHeader({ empresasDetalhes, onSuccess }: Props) {
  ...
  return (
    <>
      ...
      <form onSubmit={handleSubmit}>
        <CardHeader
          className={`flex flex-row justify-between p-2 pb-3 bg-primary gap-2 font-sans ${
            isMobile ? "rounded-t-none" : "rounded-t-[var(--radius)]"
          }`}
        >
          ...
          <Button
            ...
            disabled={isLoading} // ⬅️ evita múltiplos submits
          >
