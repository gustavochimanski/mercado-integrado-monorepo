...
export default function AdicionarBannerModal({
  ...
  const handleSave = async () => {
    ...
    try {
      if (bannerEdit) {
        // Edição: sempre usa JSON (não permite trocar imagem)
        const jsonBody = {
          nome,
          tipo_banner: tipo,
          ativo: ativo ? 1 : 0,
        ...
        await update.mutateAsync({ id: bannerEdit.id, body: jsonBody });
      } else {
        // Criação: sempre usa FormData (imagem é obrigatória)
        if (!imagemFile) {
          toast({ title: "Erro", description: "A imagem é obrigatória para criar um banner." });
          return;
        }
